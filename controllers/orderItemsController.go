...
//			c.JSON(http.StatusOK, gin.H{"Data": orderItemPack})
//		}
//	}
func CreateOrderItem() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// c.JSON("data":orderItemPack)

		// Check if Order_items is empty
		fmt.Println("Table id", orderItemPack.Table_id)
		fmt.Println("Items:", orderItemPack.Order_items)
		if len(orderItemPack.Order_items) == 0 {
			// c.JSON(gin.H{"data":o})
			c.JSON(http.StatusBadRequest, gin.H{"error": "No order items provided"})
			return
		}
		...
		// Initialize the slice for order items
		orderItemsToBeInserted := []interface{}{}
		order.Table_id = orderItemPack.Table_id
		order_id := orderItemOrderCreater(order) // Assuming this function works correctly
		...
		for _, orderItem := range orderItemPack.Order_items {
			...
			if validationErr != nil {
				// Skip invalid items and continue with the next one
				continue
			}

			// Set various fields for the order item
			...
			// Append valid order item to the slice
			orderItemsToBeInserted = append(orderItemsToBeInserted, orderItem)
		}

		// Check if there are any valid order items to insert
		if len(orderItemsToBeInserted) == 0 {
			c.JSON(http.StatusBadRequest, gin.H{"error": "No valid order items to insert"})
			return
		}

		// Insert the valid order items into the database
		...
		// Return the inserted order items
